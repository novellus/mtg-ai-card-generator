...
MTG_REMINDER_TEXT = [
    ...
    rm(fr'phases out',                                        fr"While it's phased out, (?:they're|it's) treated as though (?:they|it) doe?s?n't exist. (?:They|It|Each 1) phases? in before (?:that player|you|its controller) untaps? during (?:their|your) next untap step\."),
...
REMINDER_REGEX = fr"\((?:{REMINDER_REGEX})(?:[ ,\.]*(?:{REMINDER_REGEX}))*\)"
    ...
    if card['main_text'] is not None:
        ...
        for x in MTG_REMINDER_TEXT:
            if x.preface:
                ...
                card['main_text'] = re.sub(fr'\({x.reminder}\)', '', card['main_text'])
