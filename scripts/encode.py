...
MTG_REMINDER_TEXT = [
    ...
    rm(fr'[Cc]umulative upkeep',                              r"\(At the beginning of your upkeep, put an age counter on this permanent, then sacrifice it unless you pay its upkeep cost for each age counter on it\.(?: \{\S} can be paid with 1 mana from a snow source\.)?\)"),
    ...
    rm(fr'[Dd]eathtouch',                                     r"\(Any amount of damage (?:this|that|it|they|you) deals? to a creature is enough to destroy it\.\)"),
    rm(fr'[Dd]ecayed',                                        r"\(It can't block. When it attacks, sacrifice it at end of combat\.\)"),
    ...
    rm(fr'[Ff]lash',                                          r"\(You may cast this spell any time you could cast an instant\.\)"),
    ...
    rm(fr'[Ff]ood token',                                     r"\(It's an artifact with \"\{2\}, \{T\}, Sacrifice this artifact: You gain 3 life\.\"\)"),
    ...
    rm(fr'[Gg]oad',                                           r"\(Until your next turn, (?:this creature|that creature|this|it|you|they) attacks each combat if able and attacks a player other than you if able\.\)"),
    ...
    rm(fr'[Ii]ndestructible',                                 fr"\((?:Damage and e|E)ffects that say \"destroy\" don't destroy (?:it|this)\.?(?: (?:creature|artifact)\.(?: If its toughness is 0 or less, it's still put into its owner's graveyard\.)?)?\)"),
    ...
    rm(fr'[Kk]icker',                                         fr"\(You may (?:sacrifice (?:a|\d+) \w+s? in addition to any other costs|pay an additional (?:{MTG_SYMBOL_REGEX})+) as you cast this spell\.\)"),
    ...
    rm(fr'[Mm]ill',                                           fr"\((?:To mill a card, a player puts|You may put) the top(?: \d+)? cards? of your library into your graveyard\.\)"),
    ...
    rm(fr'[Rr]einforce',                                      fr"\((?:{MTG_SYMBOL_REGEX})+, Discard this card: Put a \+1\/\+1 counter on target creature\.\)"),
    ...
    rm(fr'[Ss]upport',                                        fr"\(Put a \+1\/\+1 counter on each of up to \d+ target creatures\.\)"),
    ...
    rm(fr'[Vv]igilance',                                      r"\(Attacking doesn't cause this creature to tap\.\)"),
