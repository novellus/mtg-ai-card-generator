...
MTG_REMINDER_TEXT = [
    ...
    rm(fr'',                                                  r"Your party consists of up to 1 each of Cleric, Rogue, Warrior, and Wizard"),
    rm(fr'',                                                  r"As a Siege enters, choose an opponent to protect it. You and others can attack it. When it's defeated, exile it, then cast it transformed.?"),
    rm(fr'',                                                  r"As this Saga enters and after your draw step, add a Lore counter(?:\. Sacrifice after III.?)?"),
    rm(fr'',                                                  r"Then exile this card. You may cast the creature later from exile"),
    ...
    rm(fr'[Aa]ftermath',                                      fr"Cast this spell only from your graveyard. Then exile it"),
    ...
    rm(fr'[Bb]ackup',                                         fr"When this creature enters the battlefield, put a \+1\/\+1 counter on target creature. If that's another creature, it gains the following ability until end of turn"),
    ...
    rm(fr'[Nn]ightbound',                                     r"If a player casts at least 2 spells during their own turn, it becomes day next turn"),
    ...
    rm(fr'[Oo]verload',                                       fr"You may cast this spell for its overload cost. If you do, change \"target\" in its text to \"each\.?\""),
    ...
    rm(fr'[Pp]owerstone token',                               fr"It's an artifact with \"(?:{MTG_SYMBOL_REGEX})+: Add (?:{MTG_SYMBOL_REGEX})+. This mana can't be spent to cast a nonartifact spell\.?\""),
    ...
    rm(fr'[Pp]rototype',                                      r"You may cast this spell with different mana cost, color, and size. It keeps its abilities and types"),
    ...
    rm(fr'[Tt]oxic',                                          r"Players dealt combat damage by this creature also get a Poison counter"),
    ...
    rm(fr'[Ww]ard',                                           fr"Whenever (?:(?:equipped|this) creature|it) becomes the target of a spell or ability an opponent controls, counter it unless that player pays (?:\d+ life|(?:{MTG_SYMBOL_REGEX})+)"),
